...
def get_strength(hand):
    ...
    first_caracter, count_of_most_common = count_cards.most_common(1)[0]
    _, count_of_second_most_common = count_cards.most_common(2)[1]
    num_of_jokers = 0
    if 'J' in count_cards:
        num_of_jokers = count_cards['J']
    if first_caracter != 'J':
        count_of_most_common += num_of_jokers
    else:
        count_of_second_most_common += num_of_jokers
        count_of_most_common = count_of_second_most_common
        if len(count_cards) == 2: return 5
        count_of_second_most_common = count_cards.most_common(3)[2] 
